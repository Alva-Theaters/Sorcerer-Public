...
class InfluenceCPV:
    ...
    Then, we need to do the things to the stuff:
        ...
        3. Release the channels that are no longer inside so they don't get stuck 
           (Release class).
    '''
...
class SetGroups:
    ...
    def _harmonize_influence_field(self, light_objects, influencers, strengths):
        ...
        for light, influence_data in light_influence_map.items():
            blended_value = sum(
                data[1] * getattr(data[0], self.influencer.property_name, 0)
...
class Initialize:
    ...
    def _determine_initiate_value(self, input_value=None):
        return input_value if input_value else getattr(self.influencer.parent, self.influencer.property_name)
