...
class Parameters:
    ...
    def find_parameter_classes(self):
        parameters = []
        for param_class in spy.data.lighting.parameter_types.values():
...
def draw_parameters_mini(context, layout, active_object, use_slider=False, expand=True, text=True):
    ...
    if ao.pan_tilt_is_on and hasattr(ao, 'object_identities_enum') and ao.object_identities_enum == 'Fixture':
