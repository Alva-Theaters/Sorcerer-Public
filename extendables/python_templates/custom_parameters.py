...
class CPV_FP_custom_gobo(spy.types.FixtureParameter): # FP stands for FixtureParameter.
    ...
    def update(controller, context):
        spy.utils.update_cpv(controller, context, CPV_FP_custom_gobo) # On update, this sends the appropriate data to the engine.
...
class CPV_FP_custom_gobo_speed(spy.types.FixtureParameter):
    ...
    def update(controller, context):
        spy.utils.update_cpv(controller, context, CPV_FP_custom_gobo_speed)
...
def register():
    for cls in parameters:
        spy.utils.register_class(cls) # Remember this uses spy.utils.register_class, NOT Blender's bpy version.
